...
namespace ArcSoftFace
{
    public partial class FaceForm : Form
    {
        ...
        /// 红外（IR）活体阈值
        /// </summary>
        private float thresholdIr = 0.5f;
        ...
        /// 活体中图像质量阈值
        /// </summary>
        private float thresholdFq = 0.65f;

        /// <summary>
        /// 睁眼幅度阈值
        /// </summary>
        private float eyeOpenThreshold = 0.5f;

        /// <summary>
        /// 嘴巴幅度阈值
        /// </summary>
        private float mouthCloseThreshold = 0.5f;

        /// <summary>
        /// 佩戴眼镜阈值
        /// </summary>
        private float wearGlassesThreshold = 0.5f;
        ...
        private void InitEngines()
        {
            try
            {
                ...
                //Video模式下检测脸部的角度优先值
                ASF_OrientPriority videoDetectFaceOrientPriority = ASF_OrientPriority.ASF_OP_0_ONLY;
                //Image模式下检测脸部的角度优先值
                ASF_OrientPriority imageDetectFaceOrientPriority = ASF_OrientPriority.ASF_OP_0_ONLY;
                //最大需要检测的人脸个数
                int detectFaceMaxNum = 6;
                //引擎初始化时需要初始化的检测功能组合
                int combinedMask = FaceEngineMask.ASF_FACE_DETECT | FaceEngineMask.ASF_FACERECOGNITION | FaceEngineMask.ASF_AGE | FaceEngineMask.ASF_GENDER | FaceEngineMask.ASF_IMAGEQUALITY | FaceEngineMask.ASF_MASKDETECT;
                ...
                //设置人脸属性阈值
                imageEngine.ASFSetFaceAttributeParam(eyeOpenThreshold, mouthCloseThreshold, wearGlassesThreshold);
                ...
                //设置人脸属性阈值
                videoEngine.ASFSetFaceAttributeParam(eyeOpenThreshold, mouthCloseThreshold, wearGlassesThreshold);
                ...
                //设置活体阈值
                videoRGBImageEngine.ASFSetLivenessParam(thresholdRgb, thresholdIr, thresholdFq);
                //设置人脸属性阈值
                videoRGBImageEngine.ASFSetFaceAttributeParam(eyeOpenThreshold, mouthCloseThreshold, wearGlassesThreshold);
                ...
                //设置活体阈值
                videoIRImageEngine.ASFSetLivenessParam(thresholdRgb, thresholdIr, thresholdFq);
                //设置人脸属性阈值
                videoIRImageEngine.ASFSetFaceAttributeParam(eyeOpenThreshold, mouthCloseThreshold, wearGlassesThreshold);
        ...
        private void ChooseImg(object sender, EventArgs e)
        {
            try
            {
                ...
                if (openFileDialog.ShowDialog().Equals(DialogResult.OK))
                {
                    ...
                    {
                        MRECT faceRect = multiFaceInfo.faceRects[i];
                        int orient = multiFaceInfo.faceOrients[i];
                        MRECT foreheadRect = multiFaceInfo.foreheadRects[i];
                        int age = 0;
                        //年龄检测
                        if (retCode_Age != 0)
                        {
                        ...
                        roll = multiFaceInfo.face3DAngle.roll[i];
                        pitch = multiFaceInfo.face3DAngle.pitch[i];
                        yaw = multiFaceInfo.face3DAngle.yaw[i];
                        ...
                        maskTemp[i] = isMask;
                        mrectTemp[i] = faceRect;
                        ageTemp[i] = age;
                        genderTemp[i] = gender;
                        AppendText(string.Format("{0} - 第{1}人脸坐标:[left:{2},top:{3},right:{4},bottom:{5}], orient:{6}, faceIsWithinBoundary:{7}, \r\nroll:{8},pitch:{9},yaw:{10},\r\n额头区域:[left:{11},top:{12},right:{13},bottom:{14}],\r\nwearGlasses:{15},leftEyeOpen:{16},rightEyeOpen:{17},mouthClose:{18},\r\nAge:{19},Gender:{20},Mask:{21}\r\n--------------------------------------------------------",
                            DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"), i, faceRect.left, faceRect.top, faceRect.right, faceRect.bottom, orient, multiFaceInfo.faceIsWithinBoundary[i],
                            roll, pitch, yaw, foreheadRect.left, foreheadRect.top, foreheadRect.right, foreheadRect.bottom, multiFaceInfo.faceAttributeInfo.wearGlasses[i], multiFaceInfo.faceAttributeInfo.leftEyeOpen[i], 
                            multiFaceInfo.faceAttributeInfo.rightEyeOpen[i], multiFaceInfo.faceAttributeInfo.mouthClose[i],
                            age, (gender >= 0 ? gender.ToString() : ""), (isMask ? "mask" : "no mask")));
                    }
                    AppendText(string.Format("--------------检测结束，时间:{0}--------------", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:ms")));
                    AppendText("");
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int faceIndex = 0; faceIndex < multiFaceInfo.faceNum; faceIndex++)
                    {
                        ...
                        {
                            //绘制额头区域
                            MRECT fRect = multiFaceInfo.foreheadRects[faceIndex];
                            float fx = fRect.left * offsetX;
                            float fWidth = fRect.right * offsetX - fx;
                            float fy = fRect.top * offsetY;
                            float fHeight = fRect.bottom * offsetY - fy;
                            g.DrawRectangle(Pens.Blue, fx, fy, fWidth, fHeight);

                            //绘制属性
                            string attributeInfo = string.Format("faceIsWithinBoundary:{0}, \r\nroll:{1},pitch:{2},yaw:{3}, \r\nwearGlasses:{4},\r\nleftEyeOpen:{5},\r\nrightEyeOpen:{6},\r\nmouthClose:{7}",
                            multiFaceInfo.faceIsWithinBoundary[faceIndex],
                            multiFaceInfo.face3DAngle.roll[faceIndex], multiFaceInfo.face3DAngle.pitch[faceIndex], multiFaceInfo.face3DAngle.yaw[faceIndex],
                            multiFaceInfo.faceAttributeInfo.wearGlasses[faceIndex], multiFaceInfo.faceAttributeInfo.leftEyeOpen[faceIndex],
                            multiFaceInfo.faceAttributeInfo.rightEyeOpen[faceIndex], multiFaceInfo.faceAttributeInfo.mouthClose[faceIndex]);
                            g.DrawString(attributeInfo, font, greenBrush, x, y + 5);
        ...
        /// </summary>
        private void txtIr_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                textBoxKeyUp(txtIr, ref thresholdIr, 0.5f);
                //更新活体阈值
                int retCode = videoIRImageEngine.ASFSetLivenessParam(thresholdRgb, thresholdIr, thresholdFq);
        ...
        private void txtRgb_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                //更新活体阈值
                int retCode = videoRGBImageEngine.ASFSetLivenessParam(thresholdRgb, thresholdIr, thresholdFq);
        ...
        /// <summary>
        /// 活体图像质量检测阈值文本框抬起事件，不正确改为默认值
        /// </summary>
        private void txtLivQua_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                textBoxKeyUp(txtLivFq, ref thresholdFq, 0.65f);
                //更新活体阈值
                int retCode = videoRGBImageEngine.ASFSetLivenessParam(thresholdRgb, thresholdIr, thresholdFq);
                AppendText(string.Format("活体图像质量检测阈值{0},阈值:{1}", retCode.Equals(0) ? "成功" : "失败,接口返回值" + retCode, thresholdFq));
            }
            catch (Exception ex)
            {
                LogUtil.LogInfo(GetType(), ex);
        ...
        /// </summary>
        private void txtImgQua_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        /// </summary>
        private void txtImgMask_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        /// </summary>
        private void txtImgNoMask_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void textBoxKeyUp(TextBox textBox,ref float value,float defaultValue)
        {
            try
            {
                //如果输入的内容不正确改为默认值
