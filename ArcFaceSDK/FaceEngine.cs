...
namespace ArcFaceSDK
{
    ...
    public class FaceEngine
    {
        ...
        {
            pEngine = IntPtr.Zero;
            if (detectFaceMaxNum < 1 || detectFaceMaxNum > 10)  ///已经支持50，大部分场景设置为10即可，这里暂时不修改
            {
                detectFaceMaxNum = 10;
            }
        ...
        /// <summary>
        /// 设置人脸属性阈值：取值范围[0-1]内部默认均为0.5， 用户可以根据实际需求，设置不同的阈值
        /// </summary>
        /// <param name="eyeOpenThreshold">睁眼幅度阈值，阈值越大睁眼幅度越大</param>
        /// <param name="mouthCloseThreshold">张嘴幅度阈值，阈值越大，张嘴幅度越小</param>
        /// <param name="wearGlassesThreshold">佩戴眼镜阈值</param>
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFSetFaceAttributeParam(float eyeOpenThreshold = 0.5f, float mouthCloseThreshold = 0.5f, float wearGlassesThreshold = 0.5f)
        {
            int retCode = -1;
            ASF_FaceAttributeThreshold faceAttributeThreshold = new ASF_FaceAttributeThreshold();
            //对应设置阈值
            faceAttributeThreshold.eyeOpenThreshold = (eyeOpenThreshold >= 0 && eyeOpenThreshold <= 1) ? eyeOpenThreshold : 0.5f;
            faceAttributeThreshold.mouthCloseThreshold = (mouthCloseThreshold >= 0 && mouthCloseThreshold <= 1) ? mouthCloseThreshold : 0.5f;
            faceAttributeThreshold.wearGlassesThreshold = (wearGlassesThreshold >= 0 && wearGlassesThreshold <= 1) ? wearGlassesThreshold : 0.5f;
            IntPtr pFaceAttributeThreshold = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_FaceAttributeThreshold>());
            MemoryUtil.StructureToPtr(faceAttributeThreshold, pFaceAttributeThreshold);
            //调用SDK接口
            retCode = ASFFunctions.ASFSetFaceAttributeParam(pEngine, pFaceAttributeThreshold);
            MemoryUtil.Free(pFaceAttributeThreshold);
            return retCode;
        }
        ...
        public int ASFDetectFaces(Image image, out MultiFaceInfo multiFaceInfo, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, ASF_DetectModel detectModel = ASF_DetectModel.ASF_DETECT_MODEL_RGB, bool isEx = false)
        {
            ...
            if (multiFaceInfo.faceNum > 0)
            {
                ...
                multiFaceInfo.faceIsWithinBoundary = new int[multiFaceInfo.faceNum];
                Marshal.Copy(multiFaceInfoStruct.faceIsWithinBoundary, multiFaceInfo.faceIsWithinBoundary, 0, multiFaceInfo.faceNum);

                multiFaceInfo.foreheadRects = new MRECT[multiFaceInfo.faceNum];
                for (int i = 0; i < multiFaceInfo.faceNum; i++)
                {
                    multiFaceInfo.foreheadRects[i] = MemoryUtil.PtrToStructure<MRECT>(multiFaceInfoStruct.foreheadRect + MemoryUtil.SizeOf<MRECT>() * i);
                }

                multiFaceInfo.faceAttributeInfo = new FaceAttributeInfo();
                //ASF_FaceAttributeInfo asfFaceAttributeInfo = MemoryUtil.PtrToStructure<ASF_FaceAttributeInfo>(multiFaceInfoStruct.faceAttributeInfo);
                ASF_FaceAttributeInfo asfFaceAttributeInfo = multiFaceInfoStruct.faceAttributeInfo; //faceAttributeInfo应定义为ASF_FaceAttributeInfo不是IntPtr
                multiFaceInfo.faceAttributeInfo.wearGlasses = new int[multiFaceInfo.faceNum];
                Marshal.Copy(asfFaceAttributeInfo.wearGlasses, multiFaceInfo.faceAttributeInfo.wearGlasses, 0, multiFaceInfo.faceNum);
                multiFaceInfo.faceAttributeInfo.leftEyeOpen = new int[multiFaceInfo.faceNum];
                Marshal.Copy(asfFaceAttributeInfo.leftEyeOpen, multiFaceInfo.faceAttributeInfo.leftEyeOpen, 0, multiFaceInfo.faceNum);
                multiFaceInfo.faceAttributeInfo.rightEyeOpen = new int[multiFaceInfo.faceNum];
                Marshal.Copy(asfFaceAttributeInfo.rightEyeOpen, multiFaceInfo.faceAttributeInfo.rightEyeOpen, 0, multiFaceInfo.faceNum);
                multiFaceInfo.faceAttributeInfo.mouthClose = new int[multiFaceInfo.faceNum];
                Marshal.Copy(asfFaceAttributeInfo.mouthClose, multiFaceInfo.faceAttributeInfo.mouthClose, 0, multiFaceInfo.faceNum);

                multiFaceInfo.face3DAngle = new Face3DAngle();
                ASF_Face3DAngle asfFace3DAngle = multiFaceInfoStruct.face3DAngleInfo;
                multiFaceInfo.face3DAngle.roll = new float[multiFaceInfo.faceNum];
                Marshal.Copy(asfFace3DAngle.roll, multiFaceInfo.face3DAngle.roll, 0, multiFaceInfo.faceNum);
                multiFaceInfo.face3DAngle.yaw = new float[multiFaceInfo.faceNum];
                Marshal.Copy(asfFace3DAngle.yaw, multiFaceInfo.face3DAngle.yaw, 0, multiFaceInfo.faceNum);
                multiFaceInfo.face3DAngle.pitch = new float[multiFaceInfo.faceNum];
                Marshal.Copy(asfFace3DAngle.pitch, multiFaceInfo.face3DAngle.pitch, 0, multiFaceInfo.faceNum);
            }
            return retCode;
        }
        ...
        public int ASFUpdateFaceData(Image image, ref MultiFaceInfo multiFaceInfo, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            ...
            if (multiFaceInfo.faceNum > 0)
            {
                ...
                byte[] allFaceRectByte = new byte[MemoryUtil.SizeOf<MRECT>() * multiFaceInfo.faceNum];
                for (int i = 0; i < multiFaceInfo.faceNum; i++)
                {
                    byte[] tempBytes = MemoryUtil.StructToBytes(multiFaceInfo.faceRects[i]);
                    tempBytes.CopyTo(allFaceRectByte, MemoryUtil.SizeOf<MRECT>() * i);
                }
                Marshal.Copy(allFaceRectByte, 0, multiFaceInfoStruct.faceRect, allFaceRectByte.Length);
                ...
                int faceDataLen = MemoryUtil.SizeOf<ASF_FaceDataInfo>();
                byte[] allFaceDataByte = new byte[faceDataLen * multiFaceInfo.faceNum];
                multiFaceInfoStruct.faceDataInfoList = MemoryUtil.Malloc(multiFaceInfo.faceNum * faceDataLen);
                for (int i = 0; i < multiFaceInfo.faceNum; i++)
                {
                    ...
                    tempBytes.CopyTo(allFaceDataByte, faceDataLen * i);
                ...
                multiFaceInfoStruct.faceIsWithinBoundary = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                Marshal.Copy(multiFaceInfo.faceIsWithinBoundary, 0, multiFaceInfoStruct.faceIsWithinBoundary, multiFaceInfo.faceNum);

                multiFaceInfoStruct.foreheadRect = MemoryUtil.Malloc(MemoryUtil.SizeOf<MRECT>() * multiFaceInfo.faceNum);
                byte[] allForeheadRectByte = new byte[MemoryUtil.SizeOf<MRECT>() * multiFaceInfo.faceNum];
                for (int i = 0; i < multiFaceInfo.faceNum; i++)
                {
                    byte[] tempBytes = MemoryUtil.StructToBytes(multiFaceInfo.foreheadRects[i]);
                    tempBytes.CopyTo(allForeheadRectByte, MemoryUtil.SizeOf<MRECT>() * i);
                }
                Marshal.Copy(allForeheadRectByte, 0, multiFaceInfoStruct.foreheadRect, allForeheadRectByte.Length);

                //int attrLen = MemoryUtil.SizeOf<ASF_FaceAttributeInfo>();
                //byte[] allFaceAttributeInfoByte = new byte[attrLen];
                //multiFaceInfoStruct.faceAttributeInfo = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_FaceAttributeInfo>());
                ASF_FaceAttributeInfo faceAttributeInfo = new ASF_FaceAttributeInfo();
                faceAttributeInfo.wearGlasses = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                Marshal.Copy(multiFaceInfo.faceAttributeInfo.wearGlasses, 0, faceAttributeInfo.wearGlasses, multiFaceInfo.faceNum);
                faceAttributeInfo.leftEyeOpen = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                Marshal.Copy(multiFaceInfo.faceAttributeInfo.leftEyeOpen, 0, faceAttributeInfo.leftEyeOpen, multiFaceInfo.faceNum);
                faceAttributeInfo.rightEyeOpen = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                Marshal.Copy(multiFaceInfo.faceAttributeInfo.rightEyeOpen, 0, faceAttributeInfo.rightEyeOpen, multiFaceInfo.faceNum);
                faceAttributeInfo.rightEyeOpen = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                Marshal.Copy(multiFaceInfo.faceAttributeInfo.mouthClose, 0, faceAttributeInfo.mouthClose, multiFaceInfo.faceNum);
                multiFaceInfoStruct.faceAttributeInfo = faceAttributeInfo;
                //byte[] tempAttrBytes = MemoryUtil.StructToBytes(faceAttributeInfo);
                //tempAttrBytes.CopyTo(allFaceAttributeInfoByte, attrLen);
                //Marshal.Copy(allFaceAttributeInfoByte, 0, multiFaceInfoStruct.faceAttributeInfo, allFaceAttributeInfoByte.Length);

                //int angleLen = MemoryUtil.SizeOf<ASF_Face3DAngle>();
                //byte[] allFace3DAngleByte = new byte[angleLen];
                //multiFaceInfoStruct.face3DAngleInfo = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_Face3DAngle>());
                ASF_Face3DAngle face3DAngle = new ASF_Face3DAngle();
                face3DAngle.roll = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<float>());
                Marshal.Copy(multiFaceInfo.face3DAngle.roll, 0, face3DAngle.roll, multiFaceInfo.faceNum);
                face3DAngle.yaw = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<float>());
                Marshal.Copy(multiFaceInfo.face3DAngle.yaw, 0, face3DAngle.yaw, multiFaceInfo.faceNum);
                face3DAngle.pitch = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<float>());
                Marshal.Copy(multiFaceInfo.face3DAngle.pitch, 0, face3DAngle.pitch, multiFaceInfo.faceNum);
                multiFaceInfoStruct.face3DAngleInfo = face3DAngle;
                //byte[] tempAngleBytes = MemoryUtil.StructToBytes(face3DAngle);
                //tempAngleBytes.CopyTo(allFace3DAngleByte, angleLen);
                //Marshal.Copy(allFace3DAngleByte, 0, multiFaceInfoStruct.face3DAngleInfo, allFace3DAngleByte.Length);
            }
            IntPtr pMultiFaceInfo = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_MultiFaceInfo>());
            ...
            if(retCode != 0)
            {
                MemoryUtil.FreeArray(pMultiFaceInfo, imageInfo.imgData, multiFaceInfoStruct.faceDataInfoList, multiFaceInfoStruct.faceID, multiFaceInfoStruct.faceOrient, 
                    multiFaceInfoStruct.faceRect, multiFaceInfoStruct.faceIsWithinBoundary, multiFaceInfoStruct.foreheadRect
                    /*multiFaceInfoStruct.faceAttributeInfo, multiFaceInfoStruct.face3DAngleInfo*/);
            ...
            if (multiFaceInfo.faceNum > 0)
            {
                ...
                multiFaceInfo.faceIsWithinBoundary = new int[multiFaceInfo.faceNum];
                Marshal.Copy(multiFaceInfoStructNew.faceIsWithinBoundary, multiFaceInfo.faceIsWithinBoundary, 0, multiFaceInfo.faceNum);

                multiFaceInfo.foreheadRects = new MRECT[multiFaceInfo.faceNum];
                for (int i = 0; i < multiFaceInfo.faceNum; i++)
                {
                    multiFaceInfo.foreheadRects[i] = MemoryUtil.PtrToStructure<MRECT>(multiFaceInfoStructNew.foreheadRect + MemoryUtil.SizeOf<MRECT>() * i);
                }

                multiFaceInfo.faceAttributeInfo = new FaceAttributeInfo();
                ASF_FaceAttributeInfo asfFaceAttributeInfo = multiFaceInfoStructNew.faceAttributeInfo;
                multiFaceInfo.faceAttributeInfo.wearGlasses = new int[multiFaceInfo.faceNum];
                Marshal.Copy(asfFaceAttributeInfo.wearGlasses, multiFaceInfo.faceAttributeInfo.wearGlasses, 0, multiFaceInfo.faceNum);
                multiFaceInfo.faceAttributeInfo.leftEyeOpen = new int[multiFaceInfo.faceNum];
                Marshal.Copy(asfFaceAttributeInfo.leftEyeOpen, multiFaceInfo.faceAttributeInfo.leftEyeOpen, 0, multiFaceInfo.faceNum);
                ...
                multiFaceInfo.face3DAngle = new Face3DAngle();
                ASF_Face3DAngle asfFace3DAngle = multiFaceInfoStructNew.face3DAngleInfo;
                multiFaceInfo.face3DAngle.roll = new float[multiFaceInfo.faceNum];
                Marshal.Copy(asfFace3DAngle.roll, multiFaceInfo.face3DAngle.roll, 0, multiFaceInfo.faceNum);
                multiFaceInfo.face3DAngle.yaw = new float[multiFaceInfo.faceNum];
                Marshal.Copy(asfFace3DAngle.yaw, multiFaceInfo.face3DAngle.yaw, 0, multiFaceInfo.faceNum);
        ...
        public int ASFProcess_IR(Image image, MultiFaceInfo multiFaceInfo, int combinedMask, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_GRAY, bool isEx = false)
        {
            ...
            //释放内存
            MemoryUtil.FreeArray(pMultiFaceInfo, imageInfo.imgData, multiFaceInfoStruct.faceDataInfoList, multiFaceInfoStruct.faceID, multiFaceInfoStruct.faceOrient, multiFaceInfoStruct.faceRect);
            MemoryUtil.FreeList(faceDataInfoIntPtrList);
        ...
        /// <summary>
        /// 设置活体阈值：取值范围[0-1]内部默认数值RGB-0.5，IR-0.5， 用户可以根据实际需求，设置不同的阈值
        /// </summary>
        /// <param name="rgbThreshold">RGB活体阈值</param>
        /// <param name="irThreshole">IR活体阈值</param>
        /// <param name="fqThreshole">活体图像质量阈值</param>
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFSetLivenessParam(float rgbThreshold = 0.5f, float irThreshole = 0.5f, float fqThreshole = 0.5f)
        {
            int retCode = -1;
            ...
            livebessThreshold.thresholdmodel_FQ = (fqThreshole >= 0 && fqThreshole <= 1) ? fqThreshole : 0.5f;
